...
class RacingCarGameSettings : RacingCarGameValidator() {
    fun inputBySettings(): GameSettings {
        ...
        if (!isDuplicateName(carNameList)) throw IllegalArgumentException("중복된 이름이 존재 합니다.")
        if (!isOverMaxNameLength(carNameList)) throw IllegalArgumentException("자동차 이름이 5자를 초과합니다.")
